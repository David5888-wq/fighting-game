...
socket.on('gameOver', (result) => {
    ...
    setTimeout(() => {
        ...
        // Запрос обновленного списка игроков
        socket.emit('requestPlayersList');
    }, 3000);
});

// Добавляем обработчик для обновления списка игроков
socket.on('playersListUpdate', (players) => {
    updatePlayersList(players);
});

// Улучшаем обработку отключения
socket.on('disconnect', () => {
    if (gameStarted) {
        const displayText = document.querySelector('#displayText');
        displayText.style.display = 'flex';
        displayText.textContent = 'Connection Lost';
...
socket.on('timerUpdate', (timeLeft) => {
    document.querySelector('#timer').innerHTML = Math.ceil(timeLeft);
});
...
function animate(timestamp) {
    ...
    if (player) {
        ...
        // Отправка состояния игрока на сервер
